<template>
  <div
    class="vault-view"
    :class="{ 'vault-view--mobile-detail': isMobile && mobileShowDetail }"
  >
    <header
      class="vault-view__topbar"
      :class="{ 'vault-view__topbar--mobile': isMobile }"
    >
      <div class="vault-view__topbar-row">
        <div class="vault-view__topbar-tools">
          ...
          <el-dropdown
            v-else
            trigger="click"
            popper-class="vault-category-dropdown"
            @command="handleCategoryCommand"
          >
            <button
              type="button"
              class="vault-view__toolbar-btn"
              :class="{ 'is-active': !!filterType }"
              :aria-label="t('vault.allCategories')"
            >
              <span
                v-if="filterType"
                class="vault-view__toolbar-btn-icon"
                :style="{ background: getPasswordTypeColor(filterType) }"
              >
                <VaultEntryTypeIcon :type="getPasswordTypeIconType(filterType)" :size="14" />
              </span>
            ...
            <template #dropdown>
              <el-dropdown-menu class="vault-category-menu">
                <el-dropdown-item command="" :class="{ 'is-active': !filterType }">
                  <span>{{ t('vault.allCategories') }}</span>
                  <svg
                    v-if="!filterType"
                    class="vault-sort-menu__check"
                    width="16"
          ...
          <button
            ...
            type="button"
            class="vault-view__toolbar-btn"
            :class="{ 'is-active': mobileSearchOpen || !!keyword }"
      ...
      <div v-if="isMobile && mobileSearchOpen" class="vault-view__mobile-search">
        <el-input
          ref="searchInputRef"
          v-model="keyword"
          :placeholder="t('vault.searchPlaceholder')"
    ...
    <div class="vault-view__split" :aria-busy="pageLoading">
      ...
      <main class="vault-view__detail-pane">
        <button
          v-if="isMobile && mobileShowDetail"
          type="button"
          class="vault-view__mobile-back"
...
function handleCategoryCommand(type: PasswordType | ''): void {
  filterType.value = type
  handleSearch()
}

async function toggleMobileSearch(): Promise<void> {
...
.vault-view {
  ...
  &__topbar-tools {
    ...
    :deep(.el-dropdown.is-open) .vault-view__toolbar-btn {
  ...
  &__topbar--mobile {
    ...
    .vault-view__topbar-row {
      gap: $spacing-sm;
    }

    .vault-view__topbar-tools {
      gap: $spacing-sm;
    }
...
.vault-category-dropdown.el-popper {
  .vault-category-menu {
    min-width: 220px;
    max-height: 60vh;
    overflow-y: auto;
...
@media (max-width: 767px) {
  .vault-view--mobile-detail {
    .vault-view__list-pane {
      display: none;
    }

    .vault-view__detail-pane {
      width: 100%;
    }
  }

  .vault-view:not(.vault-view--mobile-detail) {
    .vault-view__detail-pane {
      display: none;
    }
