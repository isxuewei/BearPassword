<template>
  <el-dialog
    ...
    :class="[
      'entry-dialog',
      {
        'entry-dialog--login': isVaultItemForm,
        'entry-dialog--mobile': isMobile
      }
    ]"
  ...
  >
    ...
    <template #footer>
      ...
      <div v-else class="entry-dialog__footer-actions">
...
const dialogWidth = computed(() => {
  ...
  return isVaultItemForm.value ? '560px' : '520px'
})
...
.entry-dialog--mobile.el-dialog {
  ...
  .el-dialog__header {
    flex-shrink: 0;
    padding: calc(#{$spacing-sm} + env(safe-area-inset-top, 0px)) $spacing-md $spacing-xs;
    margin-right: 0;
  }
  ...
  .entry-dialog__login-footer,
  .entry-dialog__footer-actions {
